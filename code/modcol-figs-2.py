...
directory = "modcol-figs-2"

def get_fig():
    fig = matplotlib.pyplot.figure(figsize=(1200/200, 1200/200))
...
def save_fig(fig, name, pdf=False):
    print(name)
    fig.savefig(f"{directory}/{name}.png", dpi=200)
    if pdf:
...
def draw_clock(ax, q, x, y, a, b, f, scale=1, highlight=False):
    ...
    ax.add_patch(matplotlib.patches.Circle(
        ...
        facecolor=light if a == 0 else white,
        edgecolor=blue if highlight else black,
            ...
            for step in presteps + list(range(ff+1)):
                ...
                for i,j in itertools.combinations(range(len(clocks)), r=2):
                    ...
                    ax.plot(
                        [2.0 + 5*x1, 2.0 + 5*x2],
                        [2.0 + 5*y1, 2.0 + 5*y2],
                        color=orange if conflict and step == "b" else light,
                        zorder=-1 if conflict and step == "b" else -2,
                        linewidth=20,
                        solid_capstyle="round",
                    )
                ...
                for i,clock in enumerate(clocks):
                    x,y = i % 2, i // 2
                    a,b = clock
                    draw_clock(ax, q, 2.0+5*x, 2.0+5*y, a, b, f/ff, scale=3, highlight=highlights[i])
